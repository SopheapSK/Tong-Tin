...
class _LoginPageState extends State<LoginPage> {
  ...
  void initState() {
    ...
    if(mounted){
      _prefs.then((f){
        ...
        var isDecs = f.getBool(PrefUtil.KEY_SORT_DECS);
        if(isDecs == null) isDecs = true;
          ...
          auth.then((res){
            if(res){
             ...
             /* Navigator.push(
              ...
              Navigator
                  .of(context)
                  .pushReplacement(new MaterialPageRoute(builder: (BuildContext context) => ListPage( title: 'Tong Tin', userID: userID, isDecs: isDecs,)));
            }
  ...
  Widget build(BuildContext context) {
    ...
    var loginButton = Padding(
      ...
      child: RaisedButton(
        ...
        child: Text(submitting ? Language.loading() : Language.login(), style: TextStyle(color: Colors.white)),
      ),
    );
  ...
  Future validateData(CRUDModel userProvider) async {
    ...
    if( pw.isEmpty || phone.isEmpty ){
      Utils.showToast(context, Language.errorFieldRequire());
      return;
    }
    ...
    setState(() {
      ...
      userID.then((f){
        ...
        _prefs.then((SharedPreferences prefs){
          ...
          var isDecs = prefs.getBool(PrefUtil.KEY_SORT_DECS);
          if(isDecs == null) isDecs = true;
         /* Navigator.push(
              context,
              MaterialPageRoute(
          ...
          Navigator
              .of(context)
              .pushReplacement(new MaterialPageRoute(builder: (BuildContext context) => ListPage( title: 'Tong Tin', userID: f.id, isDecs: isDecs,)));
        });
